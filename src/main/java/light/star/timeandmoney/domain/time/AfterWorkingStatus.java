...
public class AfterWorkingStatus implements TimeCalculator {
    @Override
    public WorkAndRestMinuteModel calculate(LocalTime localTime, FromToTimeModel workTime, FromToTimeModel restTime) {
        ...
        int fullWork = (int) ChronoUnit.MINUTES.between(workTime.getFrom(), workTime.getTo()) - fullRest;
        return new WorkAndRestMinuteModel(fullWork, fullRest);
    }
}
